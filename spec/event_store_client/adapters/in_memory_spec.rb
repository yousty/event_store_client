...
RSpec.describe EventStoreClient::InMemory do
  ...
  let(:instance) { described_class.new(mapper: EventStoreClient::Mapper::Default.new) }
  ...
  describe '#link_to' do
    ...
    before do
      allow(instance).to receive(:append_to_stream).and_call_original
    end

    it 'invokes append event to stream' do
      subject.link_to(stream_name, events)
      expect(instance).to have_received(:append_to_stream).with(stream_name, events)
    end
  end
  ...
  it 'implements the same methods as the Client' do
